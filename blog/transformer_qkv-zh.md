...
这里的这个例子，其实和大语言模型里的query，key 和 value的概念是一样的。但是的说明的是，这个例子里的query和key是一样的，但是在大语言模型里，他们其实是不太一样的，query到key的映射是一个更抽象的“查询意图”， 并不是一个真正的1对1映射。所以，用这种方法我们做出来的模型就没太大用，我们想要用更复杂的查询来寻找我们想要的信息。比如说，
...
```
加上之前的dict，我们现在就能做到用color去找苹果的值了。虽然这个例子还是很简单，但这个其实就是大语言模型attention机制的本质，用query去寻找对应的key，然后拿到最终的value。

在模型里，query和key的对应并不是绝对的，而是一个概率。我们可以理解为，在给出的query里，最有可能对应的key是哪一个。
...
对于大语言模型来说，embedding不可能只是对于一种水果的，而是每一个**token**。所以embedding里，向量的维度要大得多。这个维度，每个模型是不一样的，越大的模型维度越大，维度越大大语言模型对于每个token的语义理解（semantic understanding）就可以越精确。
...
虽然embedding的数值多多已经能表示词的真正含义，但是值得注意的是，在这里，embedding只是一个vector，大语言模型并不知道这个vector的含义。但是，意思相近的词，vector的距离是越靠近的。比如说
![vector distance](assets/transformer/vector-distance.png)
...
这个正是自注意力机制（Self-Attention）的核心

注意力的公式是这么写的：
![attention formula](assets/transformer/attention.png)

我们来详细的理解下这个公式在干啥。
首先是 Q⋅KT：我们用输入的句子的Query向量和Key的转置（transpose）来寻找词与词之间的对应关系。里边衡量得是每个词的Query对应每个词的Key的相似度。

下一步，公式里除以了sqrt(d_k): 这一步是为了防止点积结果过大，导致 Softmax 函数的梯度消失
...
整个过程如下：
![attention process](assets/transformer/attention.gif)

注意，这里的计算只是一个方向。在transformer构架中，提到了他用的是多头（multi-head）。这个是说，其实这个构架里有很多个Query，Key和Value的2维权重矩阵，每个方向从不同的角度入手分析同一个句子。有可能一个头在意的是上下文，另一个头在意的是语义。这样从多个方向入手，才能让模型更“聪明”，从而好地了解输入的真正含义。

注意下，现在输出的这个向量，他的长度和embedding vector是一致的。

## 全连接层（Feed-Forward）
从自注意层输出之后，那个输出还只包含着最基本的词义，模型还没有真正**思考**这些词义的后边，有没有包含着什么隐含的词义。这个就需要全连接层的提取。这一层其实就是一个传统的MLP。里边是夹着一个非线性激活函数（比如 GELU 或 ReLU）。它的作用是：

- **提炼信息:** 进一步处理自注意力层输出的向量，提取更深层次、更抽象的语义特征。
- **维度调整:** 调整向量的维度，为后续的层做准备。


![attention formula](assets/transformer/feed_forward.png)

## 线性层 （linear layer）
到了这一层，我们得到了一个含有最后我们想要输出的语义的向量，这个向量的长度依然和embedding vector一致。这时候，我们需要一个`解码器`,把这个embedding转化回原始的token。这个时候我们就需要另一个矩阵，这个矩阵也是预先训练好的。叫做**词汇表权重矩阵**。因为embedding代表的是token的特征，我们可以把这个矩阵理解为： **从语意特征（semantic feature）到token可能性的转化器**。
这个转化器像一个反向字典，知道了词的含义来猜单词。给出这个词义对应的单词应该是那一个（更准确地说是token）。

这个时候，我们得到了一个长度等于这个模型里所有认识的token长度的向量，向量里的每个值代表的是选择这个token的可能性。

![linear layer](assets/transformer/logit.gif)

## 最终输出
到这里，logit里已经包含了输出需要的所有信息了，我们最终只是对他做一些转化。这里有需要
...
到这一步，模型完成了一个token的输出。然后，模型会从新回到第一步，预测下一个token。直到预测的token是EOS（End-of-sequence）才会停止。
